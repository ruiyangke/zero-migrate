// CI DOC-EXAMPLE GATE (host side) — the typed `zero-migrate-engine` (host API)
...
// imports `zero-migrate-engine` (the host/facade surface). This package can
// resolve `zero-migrate-engine`'s own source, so the snippet is compiled here.
//
// HOW IT WORKS. Extract every fenced ```ts block from the guide, keep the ones
// importing `zero-migrate-engine`, rewrite the imports so tsc resolves the real
...
/** Pull every fenced ```ts block that imports `zero-migrate-engine`. */
function extractEngineBlocks(md: string): string[] {
  const blocks: string[] = [];
  const re = /```ts\n([\s\S]*?)```/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(md)) !== null) {
    if (/from\s+["']zero-migrate-engine["']/.test(m[1])) blocks.push(m[1]);
  }
  return blocks;
}

/** Rewrite a doc snippet so tsc can resolve it against the real engine source.
 *  - `from "zero-migrate-engine"` → the package's own `src/index.ts`.
...
test("doc-gate: the zero-migrate-engine embedding example in getting-started.md typechecks", () => {
  const md = readFileSync(GETTING_STARTED_DOC, "utf8");
  const blocks = extractEngineBlocks(md);
  assert.ok(
    blocks.length >= 1,
    `expected getting-started.md to carry a zero-migrate-engine embedding example; found ${blocks.length}`,
  );
  ...
  assert.equal(
    diagnostics,
    null,
    `the zero-migrate-engine embedding example in docs/getting-started.md no longer ` +
      `compiles against zero-migrate-engine.\nFix the doc (or the snippet) — do not ` +
      `weaken this gate.\n\n${diagnostics ?? ""}`,
  );
});
...
test("doc-gate REGRESSION WITNESS: a rotted engine snippet IS rejected", () => {
  const rotted = rewriteForGate(
    `import { thisEngineVerbWasRenamed } from "zero-migrate-engine";\n` +
